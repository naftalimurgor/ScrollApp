...
var GSAnalytics = function(){
    ...
    self.isDevAccount = function(linkHref) {
        if(/(.*)localhost|desktop[.]services|megakiwiqa[.]web|uat[.]pubweb|elm[.]site[.]gs|pubweb-kiwi[.]web|pubweb-published-cd04[.]web|pubweb-published[.]web|content[.]web[.]gs|content[.]gs(.*)/i.test(linkHref)) {
            return true;
        }
        return false;
    };
     ...
     * @returns {boolean}
     */
    self.isArray = function(possibleArray) {
    ...
    /**
     * Load a MediaMind tracking pixel
     * @function
     * @public
     * @name GSAnalytics#loadMMPixel
     *
     * @param {string} activityID activity ID
     *
     */
    self.loadMMPixel = function(activityID) {
        var ebRand = Math.random()+'';
        ebRand = ebRand * 1000000;
        self.log(['Loading MM pixel', activityID]);
        self.loadJS(['https://bs.serving-sys.com/BurstingPipe/ActivityServer.bs?cn=as&ActivityID=', activityID, '&rnd=', ebRand].join(''));
    };
    ...
    /**
     * Given a set of criteria, load the first matching mediamind pixel (by activityID)
     * @function
     * @public
     * @name GSAnalytics#loadFirstMatchingMMPixel
     *
     * @param {criterion[]} criteria (e.g., {match: gsAnalytics.contains, variable: s.pageName, criterion: "Goldman Sachs:Citizenship", pixel: '12345'});
     */
    self.loadFirstMatchingMMPixel = loadMatcher({
        loader: self.loadMMPixel,
        firstOnly: true
    });
     ...
     * @param {string|Array} events your Omniture events (e.g., event3,event3,event10)
     */
    self.setUpSVarForLink = function(variables, events) {
        return setUpLinkVar(variables, events);
    };
    ...
    self.handleProgressLearnMore = function(obj, name) {
        if (name) {
            ...
            self.sendCustomLink(true,'o',name,t);
            if (self.sectionName == 'Who We Are' && self.subSectionName == 'Stories of Progress') {
                self.loadFirstMatchingMMPixel([
                    {
                        criterion: 'Titan International',
                        match: self.exact,
                        pixel: '277628',
                        variable: self.pageName
                    },
                    {
                        criterion: 'New Orleans',
                        match: self.exact,
                        pixel: '277643',
                        variable: self.pageName
                    },
                    {
                        criterion: 'Human Genome Sciences',
                        match: self.exact,
                        pixel: '277631',
                        variable: self.pageName
                    },
     ...
     * @param {HTMLElement|boolean} obj The clicked-on object (i.e., the <a> tag).  If not needed, set true
     */
    self.handleLearnAboutCookies = function(obj) {
        self.fireLinkEvent(obj, {
            events: 'event47',
            linkName: 'Learn about cookies'
        });
    };
...
// function to be completed when Omniture vars completed. Called within doAnalytics function on line 2619 in gs.sitewide.js
    ...
    self.pageViewedByUser = function(doc) {
        if (doc['visibilityState'] == 'visible') {
            ...
            doc['addEventListener']('onVisibilityChange', function(state) {
                if (state == 'visible') {
                    fireWhenVisible()
                }
            })
    ...
    self.trackEmailSignup = function(address) {
        self.fireEvent({
            ...
            callback:  function(){document.subscribeForm.submit()}
        });
    };
